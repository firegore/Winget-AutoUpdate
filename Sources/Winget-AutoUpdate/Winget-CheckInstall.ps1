...
# Log initialization
[string]$LogFile = [System.IO.Path]::Combine($Script:WorkingDir, 'logs', 'install.log')
    ...
    if ($SessionID -eq 0) {
        ...
        if ($true -eq $IsServiceUI) {
            ...
            if ($explorerprocesses.Count -gt 0) {
                ...
                Write-ToLog -LogMsg "CHECK FOR APP INSTALL (System context)" -IsHeader
            }
        }
        else {
            Write-ToLog -LogMsg "CHECK FOR APP INSTALL (System context - No ServiceUI)" -IsHeader
        }
    }
    else {
        Write-ToLog -LogMsg "CHECK FOR APP INSTALL (System context - Connected user)" -IsHeader
    }
}
else {
    Write-ToLog -LogMsg "CHECK FOR APP INSTALL (User context)" -IsHeader
}
...
if (Test-Network) {
    ...
    if ($Script:Winget) {
        ...
        ### TODO
        #Get list of installed Winget Packages, only install new Packages when missing
        Write-ToLog "Checking application installations on Winget Repository named '$($Script:WingetSourceCustom)' .." "DarkYellow"
        $installedPackages = Get-WingetInstalledApps -src $Script:WingetSourceCustom
        ...
        #Run only if $outdated is populated!
            ...
            #TODO FIXME -> Add List of Apps that are possible to install
            ...
            #If White List
            if ($UseWhiteList) {
                #For each app, notify and install
                foreach ($app in $toInstall) {
                    #only install App if it isn't installed already
                    if ($installedPackages | Where-Object { $app.Id -like $_ }) {
                        Write-ToLog "$($app.Name) is being installed"
                        Install-App $app
                    }
                    #else, skip it
                    else {
            ...
            #If Black List or default
                ...
                Write-ToLog "Blacklist Mode is not supported for Installation" "Gray"
            }

            if ($InstallOK -gt 0) {
                Write-ToLog "$InstallOK apps installed ! No more installs." "Green"
            }
        }

        if ($InstallOK -eq 0 -or !$InstallOK) {
            Write-ToLog "No new installs." "Green"
        }
